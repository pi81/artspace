...
export type WordpressCmsFixtures = {
  artists: unknown;
  artworks: unknown;
  exhibitions: unknown;
};

export function createWordpressCmsProvider(fixtures: WordpressCmsFixtures): CMSProvider {
  const fixtureArtworks = WpPostsResponseSchema.parse(fixtures.artworks).map(mapWpArtwork);
  const fixtureArtists = WpPostsResponseSchema.parse(fixtures.artists).map(mapWpArtist);
  const fixtureExhibitions = WpPostsResponseSchema.parse(fixtures.exhibitions).map(mapWpExhibition);
